...
file = fopen("test.txt","r"); //edo evala san arxio ena paradigma me 8 komvous kai ta tupono pio analitika 
...
if(file==NULL){
	...
	fgets(singleline,l,file);	//diavazw tin proti grami tou arxiou gia na parw to megethos grafou
	...
	printf("\nTable with aces:\n\n");
		for(x=0;x<n;x++){
			for(y=0;y<n;y++){				//vazoume asous sto pinaka
			printf("  %d ",matrix[x][y]);
				 }
		 printf("\n");
		} 
